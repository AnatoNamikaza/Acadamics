...
class Search_Engine {
private:
	AVL<Term_Info>* Index; int TotalDocs;
public:
	Search_Engine();
...
void Search_Engine::IndexGenerator(string& file) {
	...
	Index = new AVL<Term_Info>;
	for (; j < i; j++) {
		if (word[j] != "0") {
			Term_Info* TermValue = new Term_Info;
			TermValue->SetTerm(word[j]);
			TermValue->Analyze(file);
			Index->insert_recursuve(TermValue);
		}
	}
};
...
void Search_Engine::Add_Doc_to_Index(string& file) {
	...
	if (this->Index->GetRoot()->data->GetTerm() != "\0") {
		UpdateIndex(newtopic, file);
	}
	else if (this->Index->GetRoot()->data->GetTerm() == "\0") {
		IndexGenerator(file);
	}
};
...
void Search_Engine::Ranked_list(string* QueryWords, string& file) {
	...
	AVLNode<Term_Info>* Searched;
	for (int i = 0; QueryWords[i] != ""; i++) {
		Searched = Index->SearchAddress(QueryWords[i], Index->GetRoot());
		Node<Doc_Info>* DocsNo = Searched->data->GetDocs()->GetHead()->Getnext();
		while (DocsNo != Searched->data->GetDocs()->Gettail()) {
			int j = DocsNo->GetData()->GetDocID();
			if (j > 0) {
				RList[j - 1] = j; QTerms[j - 1] = QTerms[j - 1] + 1; TFs[j - 1] = TFs[j - 1] + DocsNo->GetData()->GetTFreq();
			}
	...
	for (int i = 0; i < s; i++) {
		for (int j = i + 1; j < s; j++) {
			if (QTerms[i] < QTerms[j]) {
				...
				RList[i] = RList[j]; QTerms[i] = QTerms[j]; TFs[i] = TFs[j];
				RList[j] = temp; QTerms[j] = temp1; TFs[j] = temp2;
			}
		}
	}
	...
	for (int i = 0; i < s; i++) {
		for (int j = i + 1; j < s; j++) {
			if (QTerms[i] == QTerms[j] && TFs[i] == TFs[j]) {
				string str1, str2;
				GetDoc(RList[i], str1, file); GetDoc(RList[j], str2, file);
				if (str1 < str2) {
					int temp = RList[i], temp1 = QTerms[i], temp2 = TFs[i];
					RList[i] = RList[j]; QTerms[i] = QTerms[j]; TFs[i] = TFs[j];
					RList[j] = temp; QTerms[j] = temp1; TFs[j] = temp2;
	...
	delete[]RList, QTerms, TFs; RList = nullptr; QTerms = nullptr, TFs = nullptr;
};
