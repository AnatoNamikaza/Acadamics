...
public:
	...
	void CopyList(DList<type> const& obj);
	void insertAtEnd();
	Node<type>* GetHead();
	Node<type>* Gettail();
	void deleteAtEnd();
...
template<typename type>
void DList<type>::CopyList(DList<type> const& obj) {
	if (head->Getnext() != tail) {
		while (tail->Getprev() != head) {
			tail = tail->Getprev();
			delete tail->Getnext();
...
template<typename type>
void DList<type>::Merge(DList<type>* const obj) {
	Node<type>* tmp, * tmp2 = obj->GetHead()->Getnext();
	while (tmp2 != obj->Gettail()) {
		tmp = this->head->Getnext();
		while (tmp != this->tail) {
			if (tmp->GetData()->compare(tmp2->GetData()))
				break;
			tmp = tmp->Getnext();
			if (tmp == this->tail) {
				this->insertAtEnd();
				this->Gettail()->Getprev()->SetData(tmp2->GetData());
			}
		}
		tmp2 = tmp2->Getnext();
	}
