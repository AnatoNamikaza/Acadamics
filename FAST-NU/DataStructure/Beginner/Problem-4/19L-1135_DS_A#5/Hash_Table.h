...
struct HashItem {
	...
	HashItem() { key = 0, status = 0; value = nullptr; }
		...
		for (int i = 0; i < (capacity / 2); i++) {
			if (ListTable[i].Item.status != 0) {
				...
				if (temp[ind].Item.status == 0 || temp[ind].Item.status == 2) {
					...
					Node<HashItem<type>>* NewData= new Node<HashItem<type>>;
...
public:
	...
	HashMap() {
		...
		ListTable = new HashList<type>[capacity];
	}

	void insert(int const key, type* value) {
	...
	~HashMap() {
		...
		delete[] ListTable; ListTable = nullptr;
	}
};