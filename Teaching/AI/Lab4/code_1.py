...
class Node:
    def __init__(self, portfolio, remaining_budget, value=None):
...
def evaluate_portfolio(portfolio):
    ...
    return total_return - total_risk  # Simple evaluation: maximize return while minimizing risk

def generate_possible_moves(portfolio, remaining_budget):
    # Generate all possible combinations of investments within the remaining budget
    # For simplicity, we'll limit to selecting at most one unit of each investment option
    possible_moves = []
    for option in portfolio:
        if option.expected_return <= remaining_budget:
            new_portfolio = portfolio[:]
            new_portfolio.remove(option)
            new_remaining_budget = remaining_budget - option.expected_return
            possible_moves.append((option, new_portfolio, new_remaining_budget))
    return possible_moves

def is_terminal(portfolio, remaining_budget):
    # Terminal condition: No remaining budget or all investment options fully utilized
    return remaining_budget == 0 or len(portfolio) == 0

def minimax(node, depth, alpha, beta, maximizing_player):
    if depth == 0 or is_terminal(node.portfolio, node.remaining_budget):
        return evaluate_portfolio(node.portfolio)
...
def get_best_move(root):
    ...
    for move in generate_possible_moves(root.portfolio, root.remaining_budget):
        ...
        if value > best_value:
            best_value = value
            best_move = move[0]  # Only select the investment option, ignoring the rest of the tuple
    return best_move
...
# Initial portfolio and budget
initial_portfolio = options[:]
initial_budget = 0.15  # Initial budget of $0.15

# Create the root node
root_node = Node(portfolio=initial_portfolio, remaining_budget=initial_budget)
